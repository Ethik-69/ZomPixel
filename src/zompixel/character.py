...
class Character(pygame.sprite.Sprite):
    def __init__(self, main, name, pos, num):
        ...
        self.frames_switch = {
            "up": main.character_images[name]["walking_frames_up"],
    ...
    def select_frame(self):
        ...
        if self.action != "self_devour" and self.action != "":
            ...
            self.image = self.frames_switch[self.action][self.attacker.name][
                self.current_image
            ]
...
class Humain(Character):
    def __init__(self, main, name, pos, num):
        ...
        self.all_dying_frames = {
            "player": main.character_images[name]["attack"]["by_player"],
    ...
    def set_opposite_action(self):
        ...
        LOGGER.info("[*] Set Opposite Action For " + self.id_name)
        if self.action == "up":
            self.action = "down"
    ...
    def under_attack(self, attacker):
        ...
        LOGGER.info("[*] " + self.id_name + " Is Under Attack")
        self.is_under_attack = True
        self.attacker = attacker
        self.action = "self_devour"  # Se faire dévorer
        self.current_image = 0
        self.main.time.rebours[self.id_name].start([00, 2, 00])

    def is_dying(self):
        """Mort du citoyen"""
        LOGGER.info("[*] " + self.id_name + " Is Dying")
        self.is_alive = False
    ...
    def going_zombie(self):
        ...
        if rand <= 30:
            self.main.levels.current_level.pnj.add_zombie(
                self.main, "z_" + self.name, (self.rect.x, self.rect.y), self.num
            )
        ...
        if not self.is_under_attack:
            ...
            for obstacle in obstacles_collided:
                ...
                if self.collision_rect.colliderect(obstacle.collision_rect):
                    LOGGER.info("[*] Enemy Collide Object")
                    if (
...
class Zombie(Character):
    def __init__(self, main, name, pos, num):
        ...
        self.main.time.rebours[self.id_name].start(
            [00, 20, 00]
        )  # x temp de vie (H:M:S)
        self.ia_action_switch = {1: "up", 2: "down", 3: "left", 4: "right", 5: ""}
        self.radius = 200
    ...
    def dying(self):
        ...
        LOGGER.info("[*] " + self.name + " Is Dying")
        self.is_alive = False
    ...
    def obstacle_collide(self, obsctacles_list):
        ...
        if not self.is_feeding:
            ...
            for obstacle in obstacles_collided:
                ...
                if self.collision_rect.colliderect(obstacle.collision_rect):
                    LOGGER.info("[*] Zombie Collide Object")
                    if (
                        self.rect.x <= obstacle.collision_rect.x and self.moveX > 0
                    ):  # vas vers la gauche
                        self.action = "left"
                        self.action_switch[self.action]()
                    ...
                    if (
                        self.rect.y <= obstacle.collision_rect.y and self.moveY < 0
                    ):  # vas vers le haut
                        self.action = "down"
                        self.action_switch[self.action]()
            ...
            for enemy in enemy_hit_list:
                if not enemy.is_under_attack:
                    if self.hitbox_rect.colliderect(enemy.hitbox_rect):
                        LOGGER.info("[*] Mask Collide - Zombie")
                        enemy.under_attack(self)
                        self.is_feeding = True

    def update(self, obstacles_list, enemy_sprites):
        """Met à jour le pnj"""
        if self.main.time.rebours[
            self.id_name
        ].isFinish:  # si le rebour principal est fini le zombie meur
            self.dying()
