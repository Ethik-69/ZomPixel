...
class DataBase(object):
    ...
    def create_connection(self):
        try:
            ...
            LOGGER.info(503, "No database connection could be established.")
            self.connection = None
    ...
    def insert(self, player_name, player_point, player_time, victims):
        ...
        rdb.db(self.DB_NAME).table(self.TABLE_NAME).insert(
            [
                {
                    "name": player_name,
                    "point": player_point,
    ...
    def delete(self, player_name):
        LOGGER.info("[*] DB: Delete " + player_name)
        rdb.db(self.DB_NAME).table(self.TABLE_NAME).get(player_name).delete().run(
            self.connection
        )

    def change(self, id, player_name, player_point, player_time, victims):
        LOGGER.info("[*] DB: Change " + player_name)
        rdb.db(self.DB_NAME).table(self.TABLE_NAME).get(id).update(
            {
                "name": player_name,
                "point": player_point,
                "time": player_time,
    ...
    def count_by_name(self, player_name):
        LOGGER.info("[*] DB: Count by Name " + player_name)
        cursor = (
            rdb.db(self.DB_NAME)
            .table(self.TABLE_NAME)
            .filter(rdb.row["name"] == player_name)
            .count()
            .run(self.connection)
        )
        return cursor
    ...
    def get_id_by_name(self, player_name):
        LOGGER.info('[*] DB: Get Id by Name "' + player_name + '"')
        cursor = (
            rdb.db(self.DB_NAME)
            .table(self.TABLE_NAME)
            .filter(rdb.row["name"] == player_name)
    ...
    def make_full_insert(self, player_name, player_point, player_time, victims):
        if self.count_by_name(player_name) != 0:
            LOGGER.info("[*] DB: " + player_name + " Already in DB")
